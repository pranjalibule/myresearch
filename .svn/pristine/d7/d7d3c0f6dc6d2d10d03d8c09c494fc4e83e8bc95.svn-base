...
namespace SQS.nTier.TTM.WebAPI.Controllers
{
    ...
    public class ProjectModelController : ApiController
    {
        ...
        private string AddNewProjectModel(ProjectModelDTO objProjectModelDTO)
        {
            ...
            LoginSession ls = new LoginSession();
            ls.LoginName = objProjectModelDTO.CreatedBy;

            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            try
            {
                bool result = CheckNameExists(objProjectModelDTO.Name, null);
                if (result == true)
                {
                ...
                {
                    returnMessage = "This " + objProjectModelDTO.Name + " record already exists.";
                }
            }
            catch (Exception ex)
            {
        ...
        private bool CheckNameExists(string Name, int? ID)
        {
            ...
            ls.LoginName = "Admin";
            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            var CheckNameExists = objBusinessLayer.ProjectModelRepository.GetList(x => x.Name == Name && x.ID == ID);
        ...
        private string ModifyProjectModel(int ID, ProjectModelDTO objProjectModelDTO)
        {
            ...
            try
            {
                ...
                if (objProjectModelDTOById != null)
                {
                    ...
                    bool result = CheckNameExists(objProjectModelDTO.Name, ID);
                    if (result == true)
                    {
