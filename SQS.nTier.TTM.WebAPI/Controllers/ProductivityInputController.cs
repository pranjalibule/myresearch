...
namespace SQS.nTier.TTM.WebAPI.Controllers
{
    ...
    public class ProductivityInputController : ApiController
    {
        ...
        private string AddNewProductivityInput(ProductivityInputDTO objProductivityInputDTO)
        {
            ...
            LoginSession ls = new LoginSession();
            ls.LoginName = objProductivityInputDTO.CreatedBy;

            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            try
            {
                bool result = CheckNameExists(objProductivityInputDTO.Name, null);
                if (result == true)
                {
        ...
        private bool CheckNameExists(string Name, int? ID)
        {
            ...
            ls.LoginName = "Admin";
            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            var CheckNameExists = objBusinessLayer.ProductivityInputRepository.GetList(x => x.Name == Name && x.ID == ID);
        ...
        private string ModifyProductivityInput(int ID, ProductivityInputDTO objProductivityInputDTO)
        {
            ...
            try
            {
                ...
                if (objPInputDTOById != null)
                {
                    ...
                    bool result = CheckNameExists(objProductivityInputDTO.Name, ID);
                    if (result == true)
                    {
        ...
        {
            IList<IDName> objProductivityInputList1 = null;

            try
            {
                objProductivityInputList1 = this.GetProductivityInputIDNameList();
            }
            catch (Exception ex)
            {
                TTMLogger.Logger.LogError(String.Format("Error - {0}", ex.Message));
            ...
            return Json(objProductivityInputList1, new JsonSerializerSettings { PreserveReferencesHandling = PreserveReferencesHandling.Objects });
        }
