...
namespace SQS.nTier.TTM.WebAPI.Controllers
{
    ...
    public class TSOStatusController : ApiController
    {
        ...
        private IList<TSOStatusDTO> GetAllTSOStatus(int startingRecordNumber, int pageSize, out int totalRecords)
        {
            ...
            try
            {
                IList<TSOStatus> objTSOStatusList = objBusinessLayer.TSOStatusRepository.GetAll(startingRecordNumber, pageSize, x => x.DisplayOrder, false, out totalRecords);
        ...
        private string AddNewTSOStatus(TSOStatusDTO objTSOStatusDTO)
        {
            ...
            LoginSession ls = new LoginSession();
            ls.LoginName = objTSOStatusDTO.CreatedBy;

            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            try
            {
                bool result = CheckNameExists(objTSOStatusDTO.Name, null);
                if (result == true)
                {
        ...
        private bool CheckNameExists(string Name, int? ID)
        {
            ...
            ls.LoginName = "Admin";
            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            var CheckNameExists = objBusinessLayer.TSOStatusRepository.GetList(x => x.Name == Name && x.ID == ID);
        ...
        private string ModifyTSOStatus(int ID, TSOStatusDTO objTSOStatusDTO)
        {
            ...
            try
            {
                ...
                if (objTSOStatusDTOById != null)
                {
                    ...
                    bool result = CheckNameExists(objTSOStatusDTO.Name, ID);
                    if (result == true)
                    {
        ...
        internal IList<IDName> GetTSOStatusIDNameList()
        {
            ...
            try
            {
                objTSOStatusList = objBusinessLayer.TSOStatusRepository.GetPartial(x => new IDName { ID = x.ID, Name = x.Name }, x => x.DisplayOrder, null);
            }
            catch (Exception ex)
            {
                TTMLogger.Logger.LogError(String.Format("Error - {0}", ex.Message));
