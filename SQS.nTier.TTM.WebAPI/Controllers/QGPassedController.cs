...
namespace SQS.nTier.TTM.WebAPI.Controllers
{
    ...
    public class QGPassedController : ApiController
    {
        ...
        private string AddNewQGPassed(QGPassedDTO objQGPassedDTO)
        {
            ...
            try
            {
                bool result = CheckNameExists(objQGPassedDTO.Name, null);
                if (result == true)
                {
        ...
        private bool CheckNameExists(string Name, int? ID)
        {
            ...
            ls.LoginName = "Admin";
            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            var CheckNameExists = objBusinessLayer.QGPassedRepository.GetList(x => x.Name == Name && x.ID == ID);
        ...
        private string ModifyQGPassed(int ID, QGPassedDTO objQGPassedDTO)
        {
            ...
            try
            {
                ...
                if (objQGPassedDTOById != null)
                {
                    ...
                    bool result = CheckNameExists(objQGPassedDTO.Name, ID);
                    if (result == true)
                    {
