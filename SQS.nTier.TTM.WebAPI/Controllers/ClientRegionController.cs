...
namespace SQS.nTier.TTM.WebAPI.Controllers
{
    ...
    public class ClientRegionController : ApiController
    {
        ...
        private string AddNewClientRegion(ClientRegionDTO objClientRegionDTO)
        {
            ...
            LoginSession ls = new LoginSession();
            ls.LoginName = objClientRegionDTO.CreatedBy;

            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            try
            {
                bool result = CheckNameExists(objClientRegionDTO.Name, null);
                if (result == true)
                {
                ...
                {
                    returnMessage = "This " + objClientRegionDTO.Name + " record already exists.";
                }
            }
            catch (Exception ex)
            {
        ...
        private bool CheckNameExists(string Name, int? ID)
        {
            ...
            ls.LoginName = "Admin";
            IBusinessLayer objBusinessLayer = new BusinessLayer(ls);
            var CheckNameExists = objBusinessLayer.ClientRegionRepository.GetList(x => x.Name == Name && x.ID == ID);
        ...
        private string ModifyClientRegion(int ID, ClientRegionDTO objClientRegionDTO)
        {
            ...
            try
            {
                ...
                if (objClientRegionDTOById != null)
                {
                    ...
                    bool result = CheckNameExists(objClientRegionDTO.Name, ID);
                    if (result == true)
                    {
