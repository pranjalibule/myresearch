...
namespace SQS.nTier.TTM.WebAPI.Controllers
{
    ...
    public class LoginController : ApiController
    {
        ...
        public JsonResult<IDName> ValidateLogin([FromBody]LoginDTO loginDTO)
        {
            ...
            try
            {
                ...
                if (loginDTO.UserName != "admin")
                {
                    ...
                    {
                        objIDName = new IDName { Name = String.Format("Validation credential Error: {0}", ex.Message) };
        ...
        [EnableCors(origins: "*", headers: "*", methods: "*", SupportsCredentials = true)]
        [HttpPost, Route("v1/Login/singleSingON")]
        //[SessionAuthorize(Roles = "Admin,Account Manager,Delivery Manager,Test Manager,Team Lead,PQM,Guest")]
        [Authorize]
        public JsonResult<IDName> singleSingON()
        {
            IDName objIDName = null;
            User objUser;
            LoginSession ls = new LoginSession();
            ...
            try
            {
                ...
                {
                    objIDName = new IDName { Name = "Error: User is not authenticate or password may be expired." };
                    TTMLogger.Logger.LogError(objIDName.Name);
                }
                else
                {
                    ...
                    if (UserName != "admin")
                    {
                        try
                        {
                            ...
                            foreach (string adPath1 in adPathArr)
                            {
                                PrincipalContext pc = new PrincipalContext(ContextType.Domain, adPath1.Trim());
                                UserPrincipal up = UserPrincipal.FindByIdentity(pc, IdentityType.SamAccountName, UserName);
                                if (up != null)
                                {
                                    //connectedServer = connectedServer + ", " + up.Context.ConnectedServer + " = " + pc.Container;
                                    //if (up.Context.ConnectedServer.Contains(pc.Container))
                                    //{
                                    chkCredentials = true;
                                    adPath = adPath1;
                                    break;
                    ...
                    if (!chkCredentials && UserName != "admin")
                    {
                        objIDName = new IDName { Name = "Error: Wrong username/pwd, Please enter correct username and password" };
                        TTMLogger.Logger.LogError(objIDName.Name);
                    }
                    ...
                    {
                        objUser = objBusinessLayer.UserRepository.GetSingle(x => x.UserId.ToLower() == UserName.ToLower(), x => x.Role);
                        ...
                        {
                            if (objUser == null && UserName != "admin")
                            {
                                System.DirectoryServices.DirectoryEntry entry = new System.DirectoryServices.DirectoryEntry(String.Format("LDAP://{0}", adPath));
                                System.DirectoryServices.DirectorySearcher searcher = new System.DirectoryServices.DirectorySearcher(entry);
                                searcher.Filter = string.Format("(&(objectCategory=person)(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(sAMAccountName={0}))", UserName);
                                ...
                                if (null != results && RecordFound > 0)
                                {
                                    ...
                                    if (IsActive == true)
                                    {
                                        ...
                                        if (null != objRole)
                                        {
                                            objUser = new User { RoleId = objRole.ID, UserId = UserName, EmailID = strMail, Name = strName, Password = objUser.Password };
